...
def match_question_type(message: str) -> str:
    """Match user message to predefined question types"""
    ...
    if any(keyword in message_lower for keyword in ['educational background', 'education']):
        ...
        return 'projects'
    elif any(keyword in message_lower for keyword in ['skills', 'what skills']):
        return 'skills'
    elif any(keyword in message_lower for keyword in ['tell me about nadav', 'about nadav']):
        return 'about'
    elif any(keyword in message_lower for keyword in ['courses', 'what courses']):
        return 'courses'
    else:
        return 'custom'

def get_response(question_type: str) -> str:
    """Get response based on question type"""
    if question_type in RESPONSE_DATA:
        return RESPONSE_DATA[question_type]
    else:
        return "Don't be greedy! Please select from the available questions to learn about Nadav."

def generate_session_title(first_message: str) -> str:
    """Generate session title based on first message"""
    message_lower = first_message.lower()
    
    if 'education' in message_lower:
        return "Educational Background"
    elif 'projects' in message_lower:
        return "Nadav's Projects"
    elif 'skills' in message_lower:
        return "Technical Skills"
    elif 'about' in message_lower:
        return "About Nadav"
    elif 'courses' in message_lower:
